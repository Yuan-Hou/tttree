...
from app.auth.users import bootstrap_and_load_users
...
# 登录前端构建产物(frontend-auth/dist)。部署期挂在 /login 下,与 /app、/auth 同源 →
# token 经 hash 交接成立。开发期走独立 Vite(:5174),无产物时跳过。
_LOGIN_DIR = Path(__file__).resolve().parents[2] / "frontend-auth" / "dist"
...
async def lifespan(app: FastAPI):
    ...
    async with async_session() as session:
        # 用户系统:DB 空则从 auth_users.toml 种子,随后整表载入进程缓存(供同步鉴权热路径)。
        await bootstrap_and_load_users(session)
...
# 登录前端挂在 /login(部署期同源前门)。
if (_LOGIN_DIR / "index.html").exists():
    app.mount("/login", StaticFiles(directory=str(_LOGIN_DIR), html=True), name="login")
...
@app.get("/")
async def index():
    """根路径前门:部署期(登录前端已构建)直接跳 /login/;否则退回极简测试页(纯后端/开发)。"""
    if (_LOGIN_DIR / "index.html").exists():
