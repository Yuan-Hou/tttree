...
from app.turns.retry import ENTRY_POINTS, retry_turn
...
class RetryReq(BaseModel):
    entry: str  # director_a | writer | director_b


@router.post("/{story_id}/retry")
async def post_retry(
    story_id: str, req: RetryReq, session: AsyncSession = Depends(get_session)
) -> dict:
    if req.entry not in ENTRY_POINTS:
        raise HTTPException(400, f"entry 应为 {ENTRY_POINTS} 之一")
    if await session.get(Story, story_id) is None:
        raise HTTPException(404, "story not found")
    r = await retry_turn(session, story_id, req.entry)
    if not r.ok:
        raise HTTPException(400, r.reason or "重试失败")
    return {
        "ok": True,
        "entry": r.entry,
        ...
        "blackboard": r.blackboard,
        "invalidated_scene_slugs": r.invalidated_scene_slugs,
        "new_scene_slugs": r.new_scene_slugs,
    }
