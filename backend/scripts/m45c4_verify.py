...
async def main() -> None:
    ...
    client = get_client()
    r1 = await client.chat.completions.create(model=settings.deepseek_model, messages=reused, temperature=0.85)
    r2 = await client.chat.completions.create(model=settings.deepseek_model, messages=reused, temperature=0.85)
