  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2">
        <span
          className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-md font-mono text-[12px] ${
            d.status === "pending" ? "bg-sunken text-ink-faint" : "bg-accent-soft text-accent-ink"
          }`}
        >
          {d.glyph}
        </span>
      ...
      <div className="mt-2 flex items-center gap-2">
        <span className={`font-mono text-[10px] ${d.status === "running" ? "text-accent-ink" : "text-ink-faint"}`}>
